...
###   vritzcoincore.org and vritzcoin.space and compares them.
...
WORKINGDIR="/tmp/vritzcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://vritzcoincore.org"
HOST2="https://vritzcoin.space"
BASEDIR="/bin/"
VERSIONPREFIX="vritzcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'vritzcoin-',
...
if [ $? -ne 0 ]; then
   echo "vritzcoin.space failed to provide signature file, but vritzcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "vritzcoin.space and vritzcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Vritzcoin Core binary release signing key installed?"
   fi
