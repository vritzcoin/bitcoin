#!/bin/bash
# Copyright (c) 2013 The Vritzcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Vritzcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Vritzcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Vritzcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Vritzcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Vritzcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Vritzcoin 0.8 datadir."
    ;;
esac
