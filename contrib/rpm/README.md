...
The RPM spec file provided here is for Vritzcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://vritzcoin.space/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Vritzcoin git repository](https://github.com/vritzcoin/vritzcoin) but are not part of
...
Currently only `Source10` is used, for the example `vritzcoin.conf` file.
...
in the vritzcoin git repository. At present only an SVG version of the Vritzcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the vritzcoin
...
needed when 0.12.1 is released, a proper fix is already in the Vritzcoin git
master branch.
...
idea to build Vritzcoin Core against a static version of that library compiled
...
The `vritzcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `vritzcoin-server` package. At this point, most RPM based
...
The `vritzcoin-server` package should have SELinux support. How to properly do
...
Vritzcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/vritzcoin/). He has been
packaging Vritzcoin for Fedora at least since 2012.
...
4. I renamed the `vritzcoin` package that contains the Qt GUI to `vritzcoin-core` as
that appears to be how the general population refers to it, in contrast to
`vritzcoin-xt` or `vritzcoin-classic`. I wanted to make sure the general population
